...
message_links = {}     # { admin_message_id: user_id }
...
async def user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # подписка инфо в ответ на forwarded (чтобы привязка работала корректно)
...
async def admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text("Ответить пользователю нужно РЕПЛАЕМ 👍")
        return

    admin_msg_id = str(update.message.reply_to_message.message_id)

    if admin_msg_id not in message_links:
        await update.message.reply_text("❌ Не смог найти пользователя (возможно старая переписка).")
        return

    user_id = message_links[admin_msg_id]

    # копируем сообщение админу → пользователю (поддерживает медиа)
